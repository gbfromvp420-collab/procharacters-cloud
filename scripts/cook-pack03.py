#!/usr/bin/env python3
"""Fetch Pack 03 Drive primes, cut 4 loops, copy to live avatar folders, update status.json."""
...
def already_ready(folder: str) -> bool:
    dest = ROOT / "frontend" / "public" / "avatar" / folder
    clips = ("idle", "teasing", "playful", "aroused")
    return all((dest / f"{clip}.mp4").is_file() and (dest / f"{clip}.mp4").stat().st_size > 10_000 for clip in clips)
...
def cook(entry: dict) -> None:
    ...
    if os.environ.get("FORCE") != "1" and already_ready(folder):
        print(f"skip {who} \u2014 4 clips already present", flush=True)
        return
    ...
    if not readme.exists():
        readme.write_text(
            ...
            f"Pack 03 \u00b7 {entry['name']} \u00b7 idle / teasing / playful / aroused\n"
        )
...
def select_batch() -> list[dict]:
    if WHO == "all":
        return list(MAP)
    if WHO in {"males", "male"}:
        return [e for e in MAP if e["gender"] == "male"]
    if WHO in {"females", "female"}:
        return [e for e in MAP if e["gender"] == "female"]
...
def main() -> None:
    ...
    for entry in batch:
        try:
            ...
            print(f"FAIL {entry['who']}: {exc}", flush=True)
            failed.append(entry["who"])
    if ok:
        update_status(ok)
    print("pack03 cook done", ok)
    if failed:
        print("pack03 cook failed", failed)
        raise SystemExit(1)
