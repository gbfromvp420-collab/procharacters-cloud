...
const shards = fs
  ...
  .filter((n) => /^h\d{2}(_\d{2})?$/.test(n) && !n.includes("."))
  .filter((n) => {
    // Prefer split shards (h00_00) over parent (h00) when both exist
    if (/^h\d{2}$/.test(n)) {
      return !fs.readdirSync(partsDir).some((o) => o.startsWith(n + "_"));
    }
    return true;
  })
  .sort();
...
if (shards.length === 0) {
  console.error("assemble-chatapp: no shards in", partsDir);
  process.exit(1);
}
