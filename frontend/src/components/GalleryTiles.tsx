...
export function CharacterTile({
  ...
  const cornerBadge = resume?.resumeCode
    ? {
        label: expiryLabel || resume.resumeCode,
        className: urgent
          ? "border-rose-400/60 bg-rose-950/80 text-rose-100"
          : "border-amber-400/50 bg-black/70 text-amber-100",
        title: urgent ? "Saved chat expiring soon" : "Saved chat",
      }
    : card.mine
      ? {
          label: "Mine",
          className: "border-violet-300/50 bg-violet-600/85 text-white",
  ...
  const heatHint = resume?.resumeCode
    ? urgent
      ? "Reclaim →"
      : dnaLabel
        ? `DNA · ${dnaLabel}`
    ...
    >
      ...
      >
        ...
        <div
          ...
          aria-hidden
        />
        {card.dedicatedPack && !resume?.resumeCode && (
          <span
            className="pointer-events-none absolute left-2.5 top-2.5 z-10 h-2 w-2 rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.9)]"
        ...
        {cornerBadge && (
          <span
            className={`pointer-events-none absolute right-2.5 top-2.5 z-10 tag backdrop-blur ${cornerBadge.className}`}
            title={cornerBadge.title}
          >
            {cornerBadge.label}
          </span>
        )}
        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-[1] bg-gradient-to-t from-black/90 via-black/45 to-transparent px-3 pb-3 pt-16 sm:px-4 sm:pb-4">
          {posterTag ? (
            <span className={`tag ${posterTag.className}`}>{posterTag.label}</span>
          ) : (
            <p className="text-[10px] uppercase tracking-[0.22em] text-brand-accent">
              {card.kind === "custom" ? "Custom" : "Signature"}
            </p>
          )}
          <h2 className="mt-1.5 text-lg font-semibold leading-tight text-white sm:text-xl">
            {card.displayName}
          </h2>
          {heatHint ? (
            <p className="mt-1 line-clamp-1 text-[10px] font-medium uppercase tracking-wide text-amber-100/90">
              {heatHint}
            ...
            <p className="mt-1 text-[10px] font-medium uppercase tracking-wide text-white/0 transition group-hover:text-white/70">
              Tap to heat →
            </p>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          {resume?.resumeCode ? (
            <Link
              ...
              className={`btn-primary btn-compact min-h-0 flex-1 ${urgent ? "ring-1 ring-rose-400/70" : ""}`}
              title={expiryLabel ? `Continue · ${expiryLabel}` : "Continue saved chat"}
            >
              Continue
            </Link>
          ) : (
            <Link href={card.ctaPath} className="btn-primary btn-compact min-h-0 flex-1">
              Chat{!compact ? ` · ${first}` : ""}
            </Link>
          )}
          {forgeHref && forgeHeatCtx ? (
            <Link
              href={forgeHref}
              className="btn-ghost btn-compact min-h-0 border-violet-400/50 px-2.5 text-violet-100"
          ...
          <Link href={card.cardPath} className="btn-ghost btn-compact min-h-0 px-2.5 text-brand-muted">
            Card
          </Link>
        </div>
      </div>
