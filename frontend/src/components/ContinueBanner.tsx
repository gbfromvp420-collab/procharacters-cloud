    ...
    >
      <div className="flex flex-col gap-3 p-3 sm:flex-row sm:items-center sm:gap-4 sm:p-4">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <Link
            href={href}
            className={`btn-primary min-h-0 flex-1 px-4 py-2.5 text-sm sm:flex-none ${
          ...
          <OverflowMenu label="More">
            ...
            >
              {showQr ? "Hide QR" : "QR for another phone"}
            </button>
            <button type="button" className="menu-item" onClick={() => void copyCode()}>
              {copied ? "Copied!" : "Copy resume code"}
            </button>
            {resumeCount > 1 ? (
              <button type="button" className="menu-item" onClick={onShowAllMyChats}>
                All my chats
              </button>
            ) : null}
          </OverflowMenu>
